...
size_t sp_packet_serialize(const sp_packet_t *pkt, uint8_t *buf, size_t buf_len)
{
    ...
    first = (uint16_t)(((pkt->ph.version & 0x7) << 13) | ((pkt->ph.type & 0x1) << 12) |
                       ((pkt->ph.sec_hdr_flag & 0x1) << 11) | (pkt->ph.apid & 0x07FF));
    ...
    if (total_len == 0)
        ...
        length_field = (uint16_t)(total_len - 1);
    ...
    if (crc_present)
    {
        ...
        size_t crc_area_len = (pkt->ph.sec_hdr_flag ? (size_t)pkt->sec_hdr_len : (size_t)0) +
...
int sp_packet_parse(sp_packet_t *out, uint8_t *buf, size_t buf_len)
{
    ...
    if (out->ph.sec_hdr_flag)
    {
        ...
        if (crc_present)
        {
            if (total_len < sec_len + 2)
                return 0; /* impossible */
            payload_len_calc = (uint16_t)(total_len - sec_len - 2);
            if (buf_len < off + payload_len_calc + 2)
                return 0;
            ...
            uint16_t crc_recv =
                ((uint16_t)buf[off + out->payload_len] << 8) | buf[off + out->payload_len + 1];
            if (crc_calc != crc_recv)
                return 0;
        }
        else
        {
            if (total_len < sec_len)
                return 0;
            payload_len_calc = (uint16_t)(total_len - sec_len);
            if (buf_len < off + payload_len_calc)
                return 0;
            out->payload_len = payload_len_calc;
            out->payload = &buf[off];
        }
    ...
    else
    {
        ...
        if (buf_len < off + out->payload_len)
            return 0;
        out->payload = &buf[off];
    }
